...
const ComponentMessage: FC<IProps> = ({ photo, name, relationBusinessType, time, targetContent, businessId, isRead, parentCommentCount, workCode }) => {
  ...
  return (
    <div className="collection-card" onCopy={(e) => { e.preventDefault(); }} onContextMenu={(e) => { e.preventDefault(); }}>
      <div className="collection-card-info">
        <div className="collection-card-info-pic" onClick={()=>{openMessage(workCode);}}>
          {readState === 0 ? <div className="read"></div> : null}
          {photo ? <img src={photo} alt="" /> : <SvgIcon type="icon-default_tx" className="avar" />}
        </div>
        <div className="collection-card-info-name">
          <div><span onClick={()=>{openMessage(workCode);}}>{name}</span>收藏了你的帖子</div>
