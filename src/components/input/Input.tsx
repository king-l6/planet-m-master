...
import InputPc from './pc/input-pc';
...
const Input = React.forwardRef(({
  ...
  const renderFavNode = () => {
     ...
     * @return {*}
     */
    return <div className={countData?.isFav ? 'active-btn' : ''} onClick={() => {
      changeCountData({
        ...countData,
      ...
      <div>{countData && countData.favoriteCount && countData.favoriteCount > 9999 ? `${Math.floor(countData?.favoriteCount / 10000)}万` : countData.favoriteCount}</div>
    </div>;
  };
  ...
  const renderLikeNode = () => {
     ...
     * @return {*}
     */
    return <div className={countData?.isLike ? 'active-btn' : ''} onClick={() => {
      changeCountData({
        ...countData,
      ...
      <div>{countData && countData.likeCount && countData.likeCount > 9999 ? `${Math.floor(countData?.likeCount / 10000)}万` : countData.likeCount}</div>
    </div>;
  };
  ...
  const renderCommentNode = () => {
    ...
    >
      ...
      <div>{countData && countData.commentCount && countData.commentCount > 9999 ? `${Math.floor(countData?.commentCount / 10000)}万` : countData.commentCount}</div>
    </div>;
  };
  ...
  const renderInputNode = () => {
    return (
      <div className="edit-wrapper" id="edit-wrapper" ref={RefEdit} onScroll={(e) => { listenEditScroll(e); }}>
        {
          IsPc() ?
            renderInputNodePc() :
            <div
  ...
  const renderPublish = () => {
    return (
      IsPc() ?
        ...
        <div className="c-input-btn-publish" onTouchStart={(e) => {
          if (isInArticle) {
            submitContent();
          } else {
            submitReplyContent();
  ...
  const renderToolBar = () => {
    return (
      <div className="tool">
        {IsPc() ?
          <span className="icon-image" onMouseDown={() => {
            DOMRefInput.current.innerText.length === 0 && pasteHtmlAtCaret();
            handleKeyUp(true);
          }}><SvgIcon type="icon-at_line" /></span> :
          <span className="icon-image" onTouchStart={() => {
            DOMRefInput.current.innerText.length === 0 && pasteHtmlAtCaret();
            handleKeyUp(true);
          }}><SvgIcon type="icon-at_line" /></span>
        }
  ...
  return <div className="c-input">
    <div>
      {
        isInput && !inputValue && state.userInfo.currentOfficial ? <div className='c-input-customerService'>
      ...
      {renderInputNode()}
      {
        isInput && inputValue ? renderPublish() : isInput ? null : countData ? <div>
          {renderCommentNode()}
          {renderLikeNode()}
          {renderFavNode()}
        </div> : null
